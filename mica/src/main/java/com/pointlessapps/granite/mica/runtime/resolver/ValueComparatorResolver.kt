...
internal object ValueComparatorResolver {
    ...
    fun Any.compareToAs(other: Any, thisType: Type, otherType: Type): Int {
        ...
        return when (otherType) {
            ...
            NumberType -> (coercedValue as Float).compareTo(other as Float)
            NumberRangeType -> (coercedValue as ClosedFloatRange).compareTo(other as ClosedFloatRange)
            IndefiniteNumberRangeType -> (coercedValue as OpenEndFloatRange).compareTo(other as OpenEndFloatRange)
            AnyType -> 0
