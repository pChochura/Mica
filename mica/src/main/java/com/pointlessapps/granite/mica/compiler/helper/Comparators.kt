...
internal fun Any?.compareTo(other: Any?): Int {
    ...
    return when {
        this is Boolean && other is Boolean -> this.compareTo(other)
        this is Char && other is Char -> this.compareTo(other)
        this is String && other is String -> this.compareTo(other)
        this is Number && other is Number -> this.compareTo(other)
        this is CharRange && other is CharRange -> this.compareTo(other)
        this is LongRange && other is LongRange -> this.compareTo(other)
        this is ClosedDoubleRange && other is ClosedDoubleRange -> this.compareTo(other)
        this is List<*> && other is List<*> -> this.compareTo(other)
        this is Set<*> && other is Set<*> -> this.compareTo(other)
        this is Map<*, *> && other is Map<*, *> -> this.compareTo(other)
        else -> throw RunTimeException { "${this.toType()} and ${other.toType()} are not comparable" }
    }
}
 ...
 * implementation that doesn't fit standard types), a [RunTimeException] is thrown.
 *
...
internal fun Number.compareTo(other: Number): Int = when (this) {
    ...
    else -> throw RunTimeException { "Cannot compare numbers of different types" }
}
