    ...
    execute = { args ->
        ...
        RealType to when {
            ...
            type.isSubtypeOf(RealType) -> type.valueAsSupertype<RealType>(value) as Double
                ...
                "toReal function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
    ...
    execute = { args ->
        ...
        BoolType to when {
            ...
            type.isSubtypeOf(BoolType) -> type.valueAsSupertype<BoolType>(value) as Boolean
                ...
                "toBool function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
    ...
    execute = { args ->
        ...
        CharType to when {
            ...
            type.isSubtypeOf(CharType) -> type.valueAsSupertype<CharType>(value) as Char
                ...
                "toChar function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
...
internal val toArrayFunction = BuiltinFunctionDeclarationBuilder.create(
    ...
    execute = { args ->
        ...
        args[0].first.superTypes.filterIsInstance<ArrayType>().first() to
                type.valueAsSupertype<ArrayType>(value) as List<*>
    },
)
    ...
    execute = { args ->
        ...
        IntRangeType to when {
            ...
            type.isSubtypeOf(RealRangeType) ->
                ...
                "toIntRange function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
...
internal val toRealRangeFunction = BuiltinFunctionDeclarationBuilder.create(
    ...
    execute = { args ->
        ...
        RealRangeType to when {
            ...
            type.isSubtypeOf(RealRangeType) -> type.valueAsSupertype<RealRangeType>(value) as ClosedDoubleRange

            else -> throw IllegalArgumentException(
                "toRealRange function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
...
internal val toCharRangeFunction = BuiltinFunctionDeclarationBuilder.create(
    ...
    execute = { args ->
        val value = args[0].second
        val type = args[0].first
        CharRangeType to when {
            type.isSubtypeOf(IntRangeType) -> (type.valueAsSupertype<IntRangeType>(value) as LongRange).let {
            ...
            type.isSubtypeOf(RealRangeType) -> (type.valueAsSupertype<RealRangeType>(value) as ClosedDoubleRange).let {
                CharRange(Char(it.start.toInt()), Char(it.endInclusive.toInt()))
            }

            else -> throw IllegalArgumentException(
                "toCharRange function cannot be applied to ${args[0].first.name}",
            )
        }
    },
)
